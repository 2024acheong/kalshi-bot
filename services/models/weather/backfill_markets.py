...
DEFAULT_STATUSES = ["closed", "settled"]


def default_weather_series_tickers() -> list[str]:
    city_codes = sorted({city for city in CITY_COORDINATES if city != "NYC"})
    return [
        series_ticker
        for city_code in city_codes
        for series_ticker in (f"KXHIGH{city_code}", f"KXLOW{city_code}")
    ]
...
def catalog_row_from_kalshi_market(market: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "category": market.get("category") or "weather",
...
def backfill_weather_markets(
    ...
    series_tickers = series_tickers or default_weather_series_tickers()
    ...
    LOGGER.info("Upserted %s weather market_catalog rows", stored)
    return stored


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Backfill closed/settled Kalshi weather markets into market_catalog."
    )
    parser.add_argument(
        "--series",
        action="append",
        help="Series ticker to backfill, e.g. KXHIGHNY. Repeatable.",
    )
    parser.add_argument(
        "--status",
        action="append",
        help="Kalshi API status to request. Repeatable. Defaults to closed, settled.",
    )
    args = parser.parse_args()
    logging.basicConfig(level=logging.INFO)
    print(backfill_weather_markets(series_tickers=args.series, statuses=args.status))
