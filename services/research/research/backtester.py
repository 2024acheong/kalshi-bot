...
class Backtester:
    ...
    def run(self) -> dict[str, Any]:
        ...
        for index, market in enumerate(timeline, start=1):
            ...
            if isinstance(self.strategy, CalibrationMispricingStrategy):
                result = self._process_calibration_mispricing_event(
                    index=index,
    ...
    def _process_mean_reversion_event(
        ...
        if position is not None:
            intent = self.strategy.evaluate_exit(position, market, as_of=market.timestamp)
            leg_index = 1
        else:
        ...
        if fill_result.fill_qty > 0:
            ...
            if position is not None:
                ...
                open_positions[market.ticker] = MeanReversionPosition(
    ...
    def _process_calibration_mispricing_event(
        self,
        index: int,
        market: MarketState,
        features: Any,
        ...
        if fill_result.fill_qty > 0:
            ...
            if position is not None:
                ...
                open_positions[market.ticker] = CalibrationMispricingPosition(
                    ticker=market.ticker,
                    side=intent.side,
                    entry_price=fill_result.fill_price,
                    entry_model_prob=intent.model_prob,
                    qty=fill_result.fill_qty,
                    opened_at=market.timestamp,
                )
